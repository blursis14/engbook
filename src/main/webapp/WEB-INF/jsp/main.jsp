...
<script type="text/javascript">
	...
	$(document).on('click', '.addBookmark', function(e) {
		...
		if (document.getElementById('logout')) { //로그인 되어있으면 폴더불러오고,폴더 선택하는 모달 보이기 
			...
			$.ajax({
				...
				success:function(data){
					$.each(data,function(key,value){
						var addContent=$("<option>"+value.folder+"</option>"); //폴더가져와서 모달에 폴더선택란에 보이기
						$("#bookmark-folder-select").append(addContent);
					})
				},
				error:function(e){
					var json=e.responseText;
	...
	//모달에서 폴더선택 후 북마크추가
	$(function(){
		$("#bookmark-folder").submit(function(){
		
		dinAndFolder["folder"]=$("#bookmark-folder-select option:selected").val(); //선택한 폴더 이름을 변수에 저장
			...
			<div class="modal fade" id="addBookmarkModal" tabindex="-1"
				...
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						...
						<div class="modal-body">
							<form id="bookmark-folder" name="bookmark-folder" method="POST"
								action="main">
								<div id="bookmark-folder-list" class="container">
									폴더선택:<select id="bookmark-folder-select" class="form-control"
										name="folder"></select>
								</div>
						</div>
						<div class="modal-footer">
